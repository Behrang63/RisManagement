...
def select_skills_rule_based(proposal_text: str) -> list[str]:
    """
    مسیریاب هوشمند و کلیدواژه‌ای مهارت‌های ممیزی اخلاق پزشکی.
    این تابع متن پروپوزال را تحلیل کرده و اسامی فایل‌های مهارتی مرتبط را از پوشه Ethic_SKILL خروجی می‌دهد.
    """
    # ۱. مهارت‌های پایه که در تمامی پروپوزال‌های زیست‌پزشکی الزامی و فعال هستند
    selected_skills = [
    ...
    # ۲. نگاشت کلیدواژه‌ها به مهارت‌های تخصصی (۸ مهارت تکمیلی)
    skill_rules = {
    ...
    # ۳. بررسی و افزودن مهارت‌ها بر اساس وجود حداقل یک کلیدواژه در متن
    for skill_file, keywords in skill_rules.items():
        if any(keyword in text_lower for keyword in keywords):
            selected_skills.append(skill_file)

    # حذف موارد تکراری و بازگرداندن لیست نهایی
...
def load_skill_contents(selected_skills: list[str], skills_dir: str = "Ethic_SKILL") -> tuple[str, list[str]]:
    """
    محتوای متنی فایل‌های مهارتی انتخاب‌شده را از پوشه Ethic_SKILL می‌خواند 
    و آن‌ها را در قالب یک رشته متنی یکپارچه جهت تزریق به System Prompt بازمی‌گرداند.
    """
